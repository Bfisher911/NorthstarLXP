...
import { ArrowRight, FileCheck2, Pencil, Plus, Workflow } from "lucide-react";
import { PageHeader } from "@/components/shell/page-header";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { Sankey, type SankeyLink, type SankeyNode } from "@/components/charts/sankey";
import { SurveyBuilderDialog } from "@/components/surveys/survey-builder-dialog";
import {
  getCourseById,
...
export default async function SurveysPage({
  ...
  const courseOptions = getCoursesForWorkspace(ws.id).map((c) => ({
    id: c.id,
    title: c.title,
  }));
                  ...
                  existing={{
                    ...
                    questions: s.questions.map((q) => ({
                      ...
                      options: q.options.map((o) => ({
                        id: o.id,
                        label: o.label,
                        triggersCourseId: o.triggersCourseId ?? null,
                      })),
                    })),
                  }}
                  trigger={
